...
const URL_PATTERNS = {
    ...
    youtube: {
        ...
        video: /^https?:\/\/(?:youtu\.be\/|(?:www\.|m\.)?youtube\.com\/(?:watch(?:\/|\?v=)|shorts\/|embed\/|v\/))([\w-]{11})(?:\?.*)?$/,
...
export interface UrlValidation {
    ...
    contentType: 'video' | 'playlist' | 'channel' | 'bangumi' | 'course' | 'live' | 'space' | 'favorites' | 'festival' | 'b23' | 'unsupported';
    videoId?: string;
...
export function validateUrl(url: string): UrlValidation {
    ...
    if (trimmedUrl.includes('bilibili.com') || trimmedUrl.includes('youtube.com') || trimmedUrl.includes('youtu.be')) {
        return {
            valid: false,
...
export function classifyError(stderr: string): DownloadError {
    ...
    if (lowerError.includes('urlopen error') ||
    ...
    if (lowerError.includes("this content isn't available") ||
    ...
    if (lowerError.includes('http error 403') ||
    ...
    if (lowerError.includes('http error 429') ||
    ...
    if (lowerError.includes('this video is private') ||
        lowerError.includes('video is private')) {
        return {
            type: 'private',
            message: '这是私密视频，无法下载',
            originalError: stderr,
    ...
    if (lowerError.includes('this video is unavailable') ||
    ...
    if (lowerError.includes('confirm your age') ||
    ...
    if (lowerError.includes('not available in your country') ||
    ...
    if (lowerError.includes('requires payment') ||
    ...
    if (lowerError.includes('仅粉丝') ||
        lowerError.includes('仅限') ||
        lowerError.includes('充电')) {
        return {
            type: 'paid',
    ...
    if (lowerError.includes('unsupported url') ||
    ...
    if (stderr.includes('"code":614') || stderr.includes('"code": 614') ||
...
class YtDlpService {
    ...
    async parseUrl(url: string): Promise<VideoInfo> {
        ...
        console.log(`[Download] 解析成功: ${data.title} (${formats.length} 种画质)${partNumber ? ` [P${partNumber}${totalParts ? `/${totalParts}` : ''}]` : ''}`);
