...
import { FileVideo, Settings, Play, FolderOpen, ArrowLeft, FileText, Check, ChevronDown, AlertCircle, X } from 'lucide-react';
                        ...
                        <div className="space-y-4">
                            ...
                            >
                                <div className="flex flex-col items-center justify-center gap-4">
                                    <div className={`p-4 rounded-full ${file ? 'bg-indigo-500/20' : 'bg-slate-800 group-hover:bg-slate-700'} transition-colors`}>
                                        <FolderOpen className={`w-8 h-8 ${file ? 'text-indigo-400' : 'text-slate-400'}`} />
                                    </div>
                                    {file ? (
                                ...
                                <div className="bg-violet-900/20 border border-violet-500/30 rounded-lg p-4 flex items-center justify-between animate-fade-in">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-violet-300/70 truncate max-w-xs">{downloadedVideoPath.split(/[\\/]/).pop()}</p>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <button
                                ...
                                <div className="flex-1 space-y-3">
                                            ...
                                            },
                                            {
                                                ...
                                                label: (
                                                    <div>
                                                        ...
                                                        <div className="text-xs text-slate-500">选择本地 .ass 或 .srt 字幕文件</div>
                                                    </div>
                                                )
                                            },
                                            {
                                    ...
                                    {subtitleMode === 'workspace' && (
                                        <div className="flex items-center gap-2 p-3 bg-indigo-500/10 border border-indigo-500/20 rounded-lg">
                                            ...
                                            <span className="text-sm text-indigo-300">将自动生成 ASS 字幕并内嵌到视频中</span>
                                        </div>
                                    )}
                                </div>
                            </div>
                        ...
                        <button
                            onClick={async () => {
                                ...
                                try {
                                    ...
                                    } else if (subtitleMode === 'workspace' && workspaceSubtitles && workspaceSubtitles.length > 0) {
                                        try {
                                            const assContent = generateAssContent(workspaceSubtitles, 'Gemini Subtitle', true, false, true);
                        ...
                        {progress && (
                            <div className="bg-slate-900/50 p-6 rounded-xl border border-slate-800 space-y-4">
                                <div className="flex justify-between items-end">
                                    <span className="text-3xl font-bold text-white">{progress.percent.toFixed(1)}<span className="text-lg text-slate-500 font-normal">%</span></span>
                                    <div className="text-right">
                                        <div className="text-indigo-400 font-mono">{progress.currentFps} <span className="text-xs text-slate-500">FPS</span></div>
