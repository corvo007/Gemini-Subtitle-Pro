...
import { ArrowLeft, FileText, Settings, Download, Play, AlertCircle, CheckCircle, FolderOpen, RefreshCw, X } from 'lucide-react';
...
export function DownloadPage({ onDownloadComplete, onGoBack, onShowLogs, onShowSettings }: DownloadPageProps) {
    const {
        status,
        videoInfo,
        progress,
    ...
    const getErrorIcon = () => {
        const iconMap: Record<string, string> = {
            'network': '🌐', 'rate_limit': '⏳', 'geo_blocked': '🌍', 'private': '🔒',
            'paid': '💰', 'login_required': '🔐', 'age_restricted': '🔞',
            'unavailable': '🚫', 'unsupported': '⚠️', 'format_unavailable': '📺', 'invalid_url': '🔗'
        };
        ...
        <div className="min-h-screen bg-slate-950 flex flex-col p-4 md:p-8">
            <div className="max-w-7xl mx-auto w-full flex-1 flex flex-col">
                ...
                <div className="max-w-3xl mx-auto w-full mt-6">
                    ...
                    {/* Error Message */}
                    {error && (
                        <div className={`flex flex-col gap-3 p-4 rounded-lg mb-4 border
                            ${errorInfo?.retryable
                        ...
                        >
                            ...
                            <div className="flex gap-2 justify-end">
                                {errorInfo?.retryable && (
                                    ...
                                    >
                                        <span className="flex items-center gap-1.5"><RefreshCw className="w-3.5 h-3.5" /> 重试</span>
                                    </button>
                                )}
                                <button
                                    onClick={reset}
                                ...
                                >
                                    <span className="flex items-center gap-1.5"><X className="w-3.5 h-3.5" /> 清除</span>
                                </button>
                            </div>
                        </div>
                    )}
                        ...
                        <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
                            ...
                            <div className="pt-4 mb-6 border-t border-white/10">
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    >
                                        <span className="flex items-center gap-1.5"><FolderOpen className="w-4 h-4" /> 更改</span>
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className="flex gap-4 justify-end pt-4 border-t border-white/10">
                                ...
                                >
                                    <span className="flex items-center gap-2"><Download className="w-4 h-4" /> 下载</span>
                                </button>
                                <button
                                ...
                                >
                                    <span className="flex items-center gap-2"><Play className="w-4 h-4 fill-current" /> 下载并生成字幕</span>
                                </button>
                            </div>
                        </div>
                    )}
                    ...
                    {status === 'completed' && outputPath && (
                        <div className="text-center p-8 bg-emerald-500/10 border border-emerald-500/30 rounded-2xl">
                            ...
                            <div className="flex gap-4 justify-center flex-wrap">
                                ...
                                >
                                    <span className="flex items-center gap-2"><FolderOpen className="w-4 h-4" /> 打开下载目录</span>
                                </button>
                                <button
                                    onClick={reset}
                                    className="px-6 py-3 bg-white/10 border border-white/20 rounded-lg text-white transition-colors hover:bg-white/15"
                                ...
                                {onDownloadComplete && (
                                    ...
                                    >
                                        <span className="flex items-center gap-2"><Play className="w-4 h-4 fill-current" /> 继续生成字幕</span>
                                    </button>
                                )}
                            </div>
                        </div>
