...
export const CustomSelect: React.FC<CustomSelectProps> = ({ value, onChange, options, className = "", icon, placeholder, direction = 'down' }) => {
    ...
    return (
        <div className={`relative ${className}`} ref={containerRef}>
            ...
            {isOpen && (
                <div className={`absolute z-50 w-full bg-slate-800 border border-slate-700 rounded-lg shadow-xl max-h-60 overflow-y-auto custom-scrollbar animate-fade-in ${direction === 'up' ? 'bottom-full mb-1' : 'mt-1'
                    }`}>
                    <div className="p-1">
                        {options.map((option) => (
                            <button
                                ...
                                className={`w-full text-left px-3 py-2 rounded-md text-sm transition-colors flex items-center justify-between ${option.disabled
                            ...
                            >
                                <div className={typeof option.label === 'string' ? "truncate" : ""}>{option.label}</div>
                                {value === option.value && !option.disabled && <CheckCircle className="w-3 h-3 text-indigo-400 shrink-0 ml-2" />}
                            </button>
                        ))}
                    </div>
                </div>
