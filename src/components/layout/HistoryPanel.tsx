...
import { X, GitCommit, FolderOpen, RotateCcw, Trash2, ChevronDown, ChevronRight } from 'lucide-react';
...
export const HistoryPanel: React.FC<HistoryPanelProps> = ({
    ...
    return (
        <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar w-full relative">
            <button onClick={onClose} className="absolute top-2 right-4 text-slate-400 hover:text-white z-10">
                <X className="w-4 h-4" />
            </button>
            ...
            <div className="border border-slate-700 rounded-xl overflow-hidden">
                ...
                >
                    <span className="flex items-center gap-2">
                        {snapshotsExpanded ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
                        本次会话快照
                        {snapshots.length > 0 && (
                ...
                {snapshotsExpanded && (
                    <div className="p-3 space-y-2">
                        {snapshots.length === 0 ? (
                            ...
                            snapshots.map((snap) => (
                                <div key={snap.id} className="bg-slate-800/50 border border-slate-700 p-3 rounded-lg flex justify-between items-center">
                                    <div>
            ...
            {isElectron && (
                <div className="border border-slate-700 rounded-xl overflow-hidden">
                    ...
                    >
                        <span className="flex items-center gap-2">
                            {historiesExpanded ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
                            历史项目
                            {histories.length > 0 && (
                        ...
                        <div className="p-3 space-y-2">
                            {histories.length === 0 ? (
                                ...
                                histories.map((history) => (
                                    <div key={history.id} className="bg-slate-800/50 border border-slate-700 p-3 rounded-lg">
                                        <div className="flex justify-between items-start">
                                            <div className="flex-1 min-w-0">
                                                <h4 className="font-medium text-slate-200 text-sm truncate">{history.fileName}</h4>
                                                <p className="text-xs text-slate-500 mt-0.5 truncate">{history.filePath}</p>
                                                <p className="text-xs text-slate-400 mt-1">
                                                    {history.subtitles.length} 行字幕 · {new Date(history.savedAt).toLocaleString()}
                                                </p>
                                            </div>
