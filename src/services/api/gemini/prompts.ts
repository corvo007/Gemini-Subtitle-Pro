...
/**
 * Parameters for timeline correction prompt
 */
export interface TimelineCorrectionParams {
  corruptedRange: {
    startIndex: number;
    endIndex: number;
    startId: string;
...
/**
 * Generate prompt for timeline correction (retry after validation failure)
 */
export const getTimelineCorrectionPrompt = (params: TimelineCorrectionParams): string => `
TIMELINE CORRECTION TASK

⚠️ CRITICAL ERROR DETECTED IN YOUR PREVIOUS OUTPUT:

Your previous refinement output has a CORRUPTED TIMELINE RANGE:
- Affected Range: Subtitle #${params.corruptedRange.startIndex + 1} (ID: ${params.corruptedRange.startId}) to #${params.corruptedRange.endIndex + 1} (ID: ${params.corruptedRange.endId})
- Total Affected: ${params.corruptedRange.affectedCount} subtitles
- Issue: A subtitle had excessive duration (>10 seconds), causing subsequent timestamps to drift until timeline suddenly regressed

YOUR TASK:
1. Listen to the audio again CAREFULLY for the affected range
2. CORRECT the timestamps based on actual speech timing
3. Ensure timestamps are CONTINUOUS (no sudden jumps or regressions)
4. Keep subtitle content unchanged, only fix timing

RULES:
- Maximum subtitle duration: 10 seconds
- Timestamps must progress forward continuously
- Use Whisper's original timing as reference

${
  params.whisperSegments
    ? `WHISPER REFERENCE (for guidance):
${JSON.stringify(params.whisperSegments, null, 2)}`
    : ''
}
...
${params.enableDiarization ? 'IMPORTANT: Preserve "speaker" field for every segment.' : ''}

Return the COMPLETE corrected subtitle array in JSON format.
`;
