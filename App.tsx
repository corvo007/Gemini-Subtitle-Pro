...
export default function App() {
  ...
  const createSnapshot = (description: string) => {
      const newSnapshot: SubtitleSnapshot = {
          ...
          description,
          subtitles: JSON.parse(JSON.stringify(subtitles)), // Deep copy
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setSubtitles(result);
      setStatus(GenerationStatus.COMPLETED);
      createSnapshot("Initial Generation");
  ...
  const handleBatchAction = async (mode: BatchOperationMode, singleIndex?: number) => {
      ...
      createSnapshot(`${actionName} (${count} segments)`);
          ...
          <div className="lg:col-span-3 lg:h-full overflow-y-auto custom-scrollbar space-y-4">
            ...
            <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-4 shadow-sm space-y-4">
                ...
                {activeTab === 'import' && (
                 ...
                 {/* Genre Display */}
                <div className="flex items-center justify-between text-xs text-slate-400 bg-slate-800/50 p-2 rounded-lg border border-slate-700/50">
                    <span className="flex items-center"><Monitor className="w-3 h-3 mr-1" /> {settings.transcriptionModel === 'whisper-1' ? 'Whisper' : 'GPT-4o'}</span>
                    <span className="flex items-center"><Clapperboard className="w-3 h-3 mr-1" /> {settings.genre}</span>
                </div>
            </div>
          ...
          <div className="lg:col-span-9 flex flex-col h-[500px] lg:h-full min-h-0">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden flex flex-col shadow-2xl relative flex-1 min-h-0">
               {showSnapshots ? (
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar relative w-full h-full" ref={subtitleListRef}>
                   {renderSubtitleList()}
                </div>
              )}
            </div>
